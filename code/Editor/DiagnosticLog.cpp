...
void CDiagnosticLog::BeginSession(const char* sessionType, const char* sessionName) {
    if (m_sessionActive) {
...
void CDiagnosticLog::EndSession() {
...
void CDiagnosticLog::LogEvent(const char* eventName) {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::LogInfo(const char* format, ...) {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::LogVector(const char* name, const XMFLOAT3& v) {
    if (!m_sessionActive) return;
    WriteIndented("%-20s (%7.3f, %7.3f, %7.3f)", name, v.x, v.y, v.z);
}

void CDiagnosticLog::LogMatrix(const char* name, const XMMATRIX& m) {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::LogAABB(const char* name, const XMFLOAT3& min, const XMFLOAT3& max) {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::LogSuccess(const char* message) {
    if (!m_sessionActive) return;
    WriteIndented("✓ %s", message);
}

void CDiagnosticLog::LogFailure(const char* reason) {
    if (!m_sessionActive) return;
    WriteIndented("✗ %s", reason);
}

void CDiagnosticLog::LogExpectedValue(const char* name, const char* value) {
    if (!m_sessionActive) return;
    WriteIndented("[Expected] %s = \"%s\"", name, value);
}

void CDiagnosticLog::LogActualValue(const char* name, const char* value) {
    if (!m_sessionActive) return;
    WriteIndented("[Actual] %s = \"%s\"", name, value);
}

bool CDiagnosticLog::VerifyEqual(const char* expected, const char* actual) {
...
void CDiagnosticLog::LogSeparator(const char* label) {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::LogSubsectionStart(const char* label) {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::LogSubsectionEnd() {
    if (!m_sessionActive) return;
...
void CDiagnosticLog::FlushToFile(const char* filepath) {
...
void CDiagnosticLog::AppendToFile(const char* filepath) {
...
void CDiagnosticLog::Clear() {
    m_buffer.clear();
    m_sessionActive = false;
    m_indentLevel = 0;
}

void CDiagnosticLog::WriteLine(const char* line) {
    m_buffer.push_back(line);
}

void CDiagnosticLog::WriteIndented(const char* format, ...) {
    va_list args;
    va_start(args, format);
...
std::string CDiagnosticLog::GetTimestamp() const {
...
std::string CDiagnosticLog::GetIndent() const {
    return std::string(m_indentLevel * 2, ' ');
}

// Convenience static logging methods
void CDiagnosticLog::Info(const char* format, ...) {
    va_list args;
    va_start(args, format);
...
void CDiagnosticLog::Warning(const char* format, ...) {
    va_list args;
    va_start(args, format);
