// ============================================
// BrdfLut_DS.ps.hlsl - BRDF LUT PS (SM 5.1)
// ============================================
...
// ============================================
// Note: BRDF LUT doesn't need any input textures or constant buffers
// The layout is still declared for consistency with the pipeline
...
float2 IntegrateBRDF(float NdotV, float roughness) {
    ...
    for (uint i = 0u; i < SAMPLE_COUNT; ++i) {
        ...
        if (NdotL > 0.0) {
            ...
            // We factor out F0: F(theta) = F0 * (1 - Fc) + Fc
            // where Fc = (1 - cos(theta))^5
            ...
            // Accumulate
            // A = integral((1 - Fc) * G_Vis dw)  (scale for F0)
            // B = integral(Fc * G_Vis dw)        (bias, doesn't depend on F0)
            A += (1.0 - Fc) * G_Vis;
            B += Fc * G_Vis;
        }
    }
