...
**架构**: UE4/5 风格的 Volumetric Lightmap，用于高质量 Per-Pixel 漫反射全局光照

**核心特性**:
- **自适应八叉树**: 根据场景几何密度自动细分 Brick
- **两级 GPU 查找**: World Position → Indirection Texture → Brick Atlas
- **L1 球谐编码**: 9 SH 系数 × RGB = 27 个 float
- **硬件 Trilinear**: 3D 纹理采样实现平滑 Per-Pixel 插值
- **Overlap Baking**: 相邻 Brick 边缘采样相同世界位置，消除接缝

**Commit**: `d9bc8ac` - Refactor: Move diffuseGIMode to CB_Frame and fix VL sampler binding
...
```cpp
struct SBrick {
    XMFLOAT3 worldMin, worldMax;      // 世界空间 AABB
    XMFLOAT3 atlasOffset;             // 在 Atlas 中的位置
    int level;                         // 八叉树层级 (0=最大)
    XMFLOAT3 shData[64][9];           // 4×4×4 体素 × 9 SH 系数
};
```
...
**Shader 实现** (`VolumetricLightmap.hlsl:SampleVolumetricLightmap`):
...
3. 点击 "Build & Bake"
4. 等待烘焙完成（有进度条和 ETA）
5. 切换 Diffuse GI Mode 到 VolumetricLightmap
...
3. **仅支持直接光烘焙**
   - 当前 `LightProbeBaker` 只烘焙直接光
   - 室内无间接光反弹效果
   - 需要实现多 Bounce GI（Path Tracing 或 Radiosity）
...
| 文件 | 用途 |
|------|------|
| `Engine/Rendering/VolumetricLightmap.h` | 核心类定义、CB 结构 |
